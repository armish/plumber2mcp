...
pr_mcp_stdio <- function(pr, 
  ...
  if (debug) {
    message("Starting MCP stdio server with ", length(tools), " tools, ",
            length(resources), " resources, and ", length(prompts), " prompts")
  }

  # Run stdio loop
  run_stdio_server(tools, resources, prompts, server_name, server_version, pr, debug)
}

#' Run the stdio message loop
#' @noRd
run_stdio_server <- function(tools, resources, prompts, server_name, server_version, pr, debug = FALSE) {
  ...
  repeat {
    ...
    tryCatch({
      ...
      # Process request
      response <- process_mcp_request(request, tools, resources, prompts, server_name, server_version, pr, debug)
...
#' @noRd
process_mcp_request <- function(request, tools, resources, prompts, server_name, server_version, pr, debug = FALSE) {
